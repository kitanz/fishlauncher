...
import type { UpdateInfo, NewsItem } from '../types'
...
async function downloadUpdate(): Promise<void> {
  ...
  try {
    const success = await window.api.downloadUpdate(updateInfo.value.file, gameDirectory.value)
...
async function launchGame(): Promise<void> {
  ...
  try {
    const success = await window.api.launchGame(gameDirectory.value)
    
    if (!success) {
      errorMessage.value = 'Failed to launch game'
    }
  } catch (error) {
